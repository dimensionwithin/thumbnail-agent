...
function planeAufnahme({ aufnahme, freigabeText, gedaechtnisText = null,
  ...
  if (!abschnitte.length) {
    return { fehler: ['Zwischen ' + ortszeitText(planungszeitpunkt) + ' und ' +
  ...
  const plan = {
    ...
    fenster: {
      ...
      versatz_am_anfang: versatzText(planungszeitpunkt),
      versatz_am_ende: versatzText(ende),
      sommerzeitwechsel_im_fenster: versatzMinuten(planungszeitpunkt) !== versatzMinuten(ende),
    },
...
function pruefePlan(plan, planungszeitpunkt, hochgeladen = null) {
  ...
  plan.termine.forEach((t, i) => {
    ...
    vorher = ms;
    if (ms < planungszeitpunkt + MINUTE_MS) {
      fehler.push(wo + ': publish_at liegt weniger als eine Minute nach dem Planungszeitpunkt.');
    }
    if (ms > ende) {
      fehler.push(wo + ': publish_at liegt spaeter als 24 Stunden nach dem Planungszeitpunkt (' +
